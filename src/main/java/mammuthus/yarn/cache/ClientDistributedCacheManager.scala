...
class ClientDistributedCacheManager() extends Logging {
  ...
  def setDistFilesEnv(env: Map[String, String]): Unit = {
    ...
    if (keys.size > 0) {
      env("MAMMUTHUS_YARN_CACHE_FILES") = keys.reduceLeft[String] {
        (acc, n) => acc + "," + n
      }
      env("MAMMUTHUS_YARN_CACHE_FILES_TIME_STAMPS") =
        timeStamps.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
      env("MAMMUTHUS_YARN_CACHE_FILES_FILE_SIZES") =
        sizes.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
      env("MAMMUTHUS_YARN_CACHE_FILES_VISIBILITIES") =
        visibilities.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
    }
  }
  ...
  def setDistArchivesEnv(env: Map[String, String]): Unit = {
    ...
    val (sizes, timeStamps, visibilities) = tupleValues.unzip3
    if (keys.size > 0) {
      env("MAMMUTHUS_YARN_CACHE_ARCHIVES") = keys.reduceLeft[String] {
        (acc, n) => acc + "," + n
      }
      env("MAMMUTHUS_YARN_CACHE_ARCHIVES_TIME_STAMPS") =
        timeStamps.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
      env("MAMMUTHUS_YARN_CACHE_ARCHIVES_FILE_SIZES") =
        sizes.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
      env("MAMMUTHUS_YARN_CACHE_ARCHIVES_VISIBILITIES") =
        visibilities.reduceLeft[String] {
          (acc, n) => acc + "," + n
        }
    }
