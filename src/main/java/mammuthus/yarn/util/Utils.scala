...
import java.util.{Properties, Locale, UUID, Random}
import java.io._
import java.net._
import mammuthus.yarn.log.Logging
import mammuthus.yarn.ApplicationConf
import java.util.concurrent.{ConcurrentHashMap, ThreadFactory, Executors, ThreadPoolExecutor}
...
import mammuthus.yarn.exception.{ApplicationUncaughtExceptionHandler, ApplicationException}
import scala.Some
...
object Utils extends Logging {
  ...
  def getApplicationClassLoader = getClass.getClassLoader
   ...
   */
  def createDirectory(root: String, namePrefix: String = "mammuthus"): File = {
    var attempts = 0
  ...
  def createTempDir(
                     ...
                     namePrefix: String = "mammuthus"): File = {
  ...
  def fetchFile(
    ...
    if (useCache) {
      ...
      copyFile(
        ...
        cachedFile,
        targetFile,
        conf.getBoolean("mammuthus.files.overwrite", false)
      )
    } else {
  ...
  private def doFetchFile(url: String,
    ...
    val targetFile = new File(targetDir, filename)
    val uri = new URI(url)
    val fileOverwrite = conf.getBoolean("mammuthus.files.overwrite", defaultValue = false)
    Option(uri.getScheme).getOrElse("file") match {
      case "http" | "https" | "ftp" =>
        ...
        val timeout = conf.getInt("mammuthus.files.fetchTimeout", 60) * 1000
 ...
 def fetchHcfsFile(
   ...
   */
  def getLocalDir(conf: ApplicationConf): String = {
    getOrCreateLocalRootDirs(conf)(0)
  }

  def isRunningInYarnContainer(conf: ApplicationConf): Boolean = {
   ...
   * set of directories.
   */
  def getOrCreateLocalRootDirs(conf: ApplicationConf): Array[String] = {
  ...
  private def getOrCreateLocalRootDirsImpl(conf: ApplicationConf): Array[String] = {
    ...
    } else if (conf.getenv("MAMMUTHUS_EXECUTOR_DIRS") != null) {
      conf.getenv("MAMMUTHUS_EXECUTOR_DIRS").split(File.pathSeparator)
    } else {
      // In non-Yarn mode (or for the driver in yarn-client mode), we cannot trust the user
      // configuration to point to a secure directory. So create a subdirectory with restricted
      // permissions under each listed directory.
      Option(conf.getenv("MAMMUTHUS_LOCAL_DIRS"))
        .getOrElse(conf.get("mammuthus.local.dir", System.getProperty("java.io.tmpdir")))
        .split(",")
        .flatMap {
        root =>
          try {
  ...
  private def getYarnLocalDirs(conf: ApplicationConf): String = {
  ...
  private var customHostname: Option[String] = sys.env.get("MAMMUTHUS_LOCAL_HOSTNAME")
  ...
  def executeAndGetOutput(
    ...
    if (exitCode != 0) {
      ...
      throw new mammuthus.yarn.exception.ApplicationException(s"Process $command exited with code $exitCode")
    }
    output.toString
  }
  ...
  def tryOrExit(block: => Unit) {
    try {
      ...
      case t: Throwable => ApplicationUncaughtExceptionHandler.uncaughtException(t)
    }
  }
   ...
   * [[scala.collection.Iterator# s i z e]] because it uses a for loop, which is slightly slower
  ...
  def resolveURIs(paths: String, testWindows: Boolean = false): String = {
    if (paths == null || paths.trim.isEmpty) {
      ...
      paths.split(",").map {
        p => Utils.resolveURI(p, testWindows)
      }.mkString(",")
    }
  }
   ...
   */
  def loadDefaultSparkProperties(conf: ApplicationConf, filePath: String = null): String = {
    val path = Option(filePath).getOrElse(getDefaultPropertiesFile())
    Option(path).foreach {
      confFile =>
        getPropertiesFromFile(confFile).filter {
          case (k, v) =>
            k.startsWith("mammuthus.")
        }.foreach {
          case (k, v) =>
  ...
  def getPropertiesFromFile(filename: String): Map[String, String] = {
    ...
    try {
      ...
      case e: IOException =>
        throw new ApplicationException(s"Failed when loading Spark properties from $filename", e)
    } finally {
      inReader.close()
    }
  }
  ...
  def getDefaultPropertiesFile(env: Map[String, String] = sys.env): String = {
    env.get("MAMMUTHUS_YARN_CLIENT_CONF_DIR")
      .orElse(env.get("MAMMUTHUS_YARN_CLIENT_HOME").map {
      t => s"$t${File.separator}conf"
    })
      .map {
      t => new File(s"$t${File.separator}mamnuthus-yarn-client-defaults.conf")
    }
      .filter(_.isFile)
   ...
   */
  def sparkJavaOpts(conf: ApplicationConf, filterKey: (String => Boolean) = _ => true): Seq[String] = {
    conf.getAll
      .filter {
   ...
   */
  def portMaxRetries(conf: ApplicationConf): Int = {
    val maxRetries = conf.getOption("mammuthus.port.maxRetries").map(_.toInt)
    if (conf.contains("mammuthus.testing")) {
  ...
  def startServiceOnPort[T](
    ...
    // Should never happen
    throw new ApplicationException(s"Failed to start service$serviceString on port $startPort")
  }
  ...
  def getMaxResultSize(conf: ApplicationConf): Long = {
    memoryStringToMb(conf.get("mammuthus.driver.maxResultSize", "1g")).toLong << 20
  }
   ...
   */
  def getSparkOrYarnConfig(conf: ApplicationConf, key: String, default: String): String = {
    val sparkValue = conf.get(key, default)
    YarnHadoopUtil.get.newConfiguration(conf).get(key, sparkValue)
  }
  ...
  def extractHostPortFromSparkUrl(sparkUrl: String): (String, Int) = {
    try {
      ...
      if (uri.getScheme != "spark" ||
        ...
        throw new ApplicationException("Invalid master URL: " + sparkUrl)
      }
      (host, port)
    } catch {
      case e: java.net.URISyntaxException =>
        throw new ApplicationException("Invalid master URL: " + sparkUrl, e)
    }
  }
