...
import mammuthus.yarn.util.YarnHadoopUtil._
...
class YarnAllocator(
  ...
  protected val memoryOverhead: Int = appConf.getInt("mammuthus.yarn.executor.memoryOverhead",
  ...
  private val launcherPool = new ThreadPoolExecutor(
    ...
    appConf.getInt("mammuthus.yarn.containerLauncherMaxThreads", 25), Integer.MAX_VALUE,
    1, TimeUnit.MINUTES,
    new LinkedBlockingQueue[Runnable](),
    new ThreadFactoryBuilder().setNameFormat("ContainerLauncher #%d").setDaemon(true).build())
  launcherPool.allowCoreThreadTimeOut(true)

  private val driverUrl = "http://" + appConf.get("mammuthus.user.app.host") + ":" + appConf.get("mammuthus.user.app.port")

  // For testing
  private val launchContainers = appConf.getBoolean("mammuthus.yarn.launchContainers", true)
...
object YarnAllocator {
  ...
  def memLimitExceededLogMessage(diagnostics: String, pattern: Pattern): String = {
    ...
    ("Container killed by YARN for exceeding memory limits." + diag
      + " Consider boosting mammuthus.yarn.executor.memoryOverhead.")
  }
}
